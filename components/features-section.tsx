...
export default function FeaturesSection() {
  ...
  return (
    <section ref={sectionRef} className="py-24 relative overflow-hidden bg-gradient-to-br from-amber-50 via-pink-50 to-purple-50">
      {/* Background */}
      <div className="absolute inset-0 -z-10">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_70%,rgba(250,204,21,0.15),transparent_50%),radial-gradient(circle_at_70%_30%,rgba(236,72,153,0.15),transparent_50%)]" />
        <div className="absolute inset-0 opacity-10 bg-[linear-gradient(45deg,#fff_25%,transparent_25%),linear-gradient(-45deg,#fff_25%,transparent_25%)] bg-[length:30px_30px]" />
        
        {/* Floating Orbs - Static with CSS animation */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        {/* Header */}
        <div className={`text-center mb-16 transition-all duration-800 ${hasAnimated ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <h2 className="text-5xl sm:text-6xl font-extrabold mb-4">
            <span className="bg-gradient-to-r from-amber-600 via-orange-500 to-pink-500 bg-clip-text text-transparent drop-shadow-md">
              Why Choose Amazon Author Partners
            </span>
          ...
          {stats.map((stat, index) => (
            ...
            >
              <div className="bg-white/90 backdrop-blur-xl rounded-2xl p-8 text-center border-2 border-white/50 shadow-lg hover:shadow-2xl hover:shadow-pink-300/40 hover:scale-105 transition-all duration-300">
                <div className={`text-5xl sm:text-6xl font-extrabold mb-3 bg-gradient-to-r ${stat.gradient} bg-clip-text text-transparent drop-shadow-lg animate-stat-pulse`}
